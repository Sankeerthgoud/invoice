  ...
  var invoiceApp = angular.module('invoiceApp', ["ngRoute"]);
	...
	invoiceApp.controller('mainController', function($scope, $http){
        ...
        console.log("on delete function ");
			...
			/*  view Invoice  */
			$http.post('http://localhost:8181/supplychain/invoice/searchInvoice?invoiceID='+invoiceNo).success(function (data) {
			...
			/*  view product  */
            $http.post('http://localhost:8181/supplychain/invoice/viewProduct/?id=' + invoiceNo)
        ...
        $scope.addItemFunc = function () {
        	...
        	console.log("in addItem function ");
    			...
    			/*  Add New product  */
                $http.post('http://localhost:8181/supplychain/invoice/addItems?invoiceID='+$scope.invoiceNo+'&productID='+$scope.productID+'&quantity='+ $scope.quantity+'&grossAmount='+$scope.grossAmount +'&tax='+$scope.tax +'&netAmount=' + $scope.netAmount)
    			.success(function (data) {
        			...
        			/*  view product  */
                    $http.post('http://localhost:8181/supplychain/invoice/viewProduct/?id=' + $scope.invoiceNo)
        			.success(function (data) {
                        $scope.productslist = data;
                        console.log($scope.productslist);
                    });
        ...
        $scope.deleteItemFunc = function () {
        	...
        	console.log("in update function ");
    			...
    			/*  delete product  */
              $http.post('http://localhost:8181/supplychain/invoice/deleteItem?invoiceID='+$scope.invoiceNo +'&productID=' + $scope.delProductId)
    			.success(function (data) {
		...
		console.log('go to delete result page');
		 $http.post('http://localhost:8181/supplychain/invoice/deleteInvoice?invoiceID=' + $scope.invoiceNo)
			.success(function (data) {
                $scope.message = data;
                console.log(data);
                });
		...
		$scope.receivedInvoicefunc = function () {
			console.log('inside received invoice');
      		$http.post('http://localhost:8181/supplychain/invoice/ReceivedInvoices?sellerID=' + $scope.sellerID)
			.success(function (data) {
		...
		$scope.sentInvoicefunc = function () {
			console.log('inside sent invoice');
      		$http.post('http://localhost:8181/supplychain/invoice/SentInvoices?sellerID=' + $scope.sellerID)
			.success(function (data) {
				console.log("inside http");
		...
		$scope.approvefunc = function (invoiceNo) {
			...
			$http.post('http://localhost:8181/supplychain/invoice/approveInvoice?invoiceID='+invoiceNo).success(function (data) {
		...
		$scope.rejectfunc = function (invoiceNo) {
			console.log('inside approve invoice');
			$http.post('http://localhost:8181/supplychain/invoice/rejectInvoice?invoiceID='+invoiceNo).success(function (data) {
		...
		$scope.viewInvoicefunc = function () {
        	...
        	console.log("in view invoice function ");
    			...
    			/*  view Invoice  */
    			$http.post('http://localhost:8181/supplychain/invoice/searchInvoice?invoiceID='+invoiceID).success(function (data) {
    			...
    			/*  view product  */
                $http.post('http://localhost:8181/supplychain/invoice/viewProduct/?id=' + invoiceID)
    			.success(function (data) {
                    $scope.productslist = data;
                    console.log($scope.productslist);
                });
